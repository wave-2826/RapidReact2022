 ...
 */
DriveStraightCommand::DriveStraightCommand(double moveInput, double rotateInput, float turnAngle, Drive* drive):
    m_moveInput(moveInput), m_rotateInput(rotateInput), m_turnAngle(turnAngle), m_drive(drive)
{
    // Use AddRequirements() here to declare subsystem dependencies
    // eg. AddRequirements(Robot::chassis.get());
    SetName("DriveStraightCommand");
}
...
void DriveStraightCommand::Execute() {
    float angle_correction = -((m_drive->GetAngle() - m_turnAngle) * 0.2f );
    ...
    //     m_drive->arcadeDrive(m_moveInput , 0.0);
    // }
    // std::cout << "Move Input: " << m_moveInput << std::endl; 
    // std::cout << "Angle Correction: " << angle_correction << std::endl; 
...
void DriveStraightCommand::End(bool interrupted) {
    // std::cout << "ENTERED DRIVE STRAIGHT COMMAND END" << std::endl; 
    // m_drive->ZeroAngle();
...
bool DriveStraightCommand::RunsWhenDisabled() const {
    return false;
}